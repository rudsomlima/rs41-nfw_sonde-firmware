...
constexpr float pipFrequencyMhz = 430.43;        // Pip tx frequency
...
constexpr float horusV3FreqTable[] = {430.43};     // Specify all horus frequencies You want (example {430.43, 434.714, 433.8};), the sonde will cycle through all of them one-by-one during a transmit cycle. Useful when flying long flights in different places of the world (Poland 430.43Mhz, most of the europe 434.714Mhz) Note - lowAltitudeFasTxMode will only use primary frequency - the first one specified.
#define HORUS_V3_CALLSIGN "PU7IOL-1"      // Payload callsign for Horus V3 mode. Note that adding every single character increases your packet size by 6 bits.
...
constexpr float horusFreqTable[] = {430.43};     // Specify all horus frequencies You want (example {430.43, 434.714, 433.8};), the sonde will cycle through all of them one-by-one during a transmit cycle. Useful when flying long flights in different places of the world (Poland 430.43Mhz, most of the europe 434.714Mhz) Note - lowAltitudeFasTxMode will only use primary frequency - the first one specified.
...
#define CALLSIGN "PU7IOL"             // Callsign used for morse and rtty
...
String foxMorseMsg = "PU7IOL FOX";                          // Morse message contents
...
String foxMorseMsgVbat = "PU7IOL FOX 11.123456 12.456789";  // Additional morse message when vBatWarnValue is met
...
*/
bool autoTemperatureCalibration = false;
...
void interfaceHandler() {
  ...
  xdataSerial.print(F("isRSM4x2:"));xdataSerial.println(rsm4x2);
  xdataSerial.print(F("isRSM4x4:"));xdataSerial.println(rsm4x4);
  xdataSerial.print(F("fwVer:"));xdataSerial.println(NFW_VERSION);
  xdataSerial.print(F("millis:"));xdataSerial.println(millis());
  xdataSerial.print(F("gpsLat:"));xdataSerial.println(gpsLat,6);
  xdataSerial.print(F("gpsLong:"));xdataSerial.println(gpsLong,6);
  xdataSerial.print(F("gpsAlt:"));xdataSerial.println(gpsAlt);
  xdataSerial.print(F("gpsSats:"));xdataSerial.println(gpsSats);
  xdataSerial.print(F("gpsHours:"));xdataSerial.println(gpsHours);
  xdataSerial.print(F("gpsMinutes:"));xdataSerial.println(gpsMinutes);
  xdataSerial.print(F("gpsSeconds:"));xdataSerial.println(gpsSeconds);
  xdataSerial.print(F("gpsSpeedKph:"));xdataSerial.println(gpsSpeedKph);
  xdataSerial.print(F("vVCalc:"));xdataSerial.println(vVCalc);
  xdataSerial.print(F("horusV3Enable:"));xdataSerial.println(horusV3Enable);
  xdataSerial.print(F("horusV3FrequencyMhz:"));xdataSerial.println(horusV3FreqTable[0],3);
  xdataSerial.print(F("aprsEnable:"));xdataSerial.println(aprsEnable);
  xdataSerial.print(F("aprsFrequencyMhz:"));xdataSerial.println(aprsFreqTable[0],3);
  xdataSerial.print(F("callsign:"));xdataSerial.println(CALLSIGN);
  xdataSerial.print(F("batV:"));xdataSerial.println(readBatteryVoltage());
  xdataSerial.print(F("mainTemperatureValue:"));xdataSerial.println(mainTemperatureValue);
  xdataSerial.print(F("humidityValue:"));xdataSerial.println(humidityValue);
  xdataSerial.print(F("pressureValue:"));xdataSerial.println(pressureValue);
  xdataSerial.print(F("err:"));xdataSerial.println(err);
  xdataSerial.print(F("warn:"));xdataSerial.println(warn);
  xdataSerial.print(F("ok:"));xdataSerial.println(ok);
}
